...
def patrol_line(cv_image):
    ...
    # 黄色线条的阈值
    #lower_yellow = np.array([0, 80, 0])#省赛
    #upper_yellow = np.array([69, 255, 255])#省赛
    lower_yellow = np.array([0, 24, 43])
    upper_yellow = np.array([62, 255, 255])
    yellow_mask = cv2.inRange(hsv, lower_yellow, upper_yellow)
    #result = cv2.bitwise_and(circular_image, circular_image, mask=yellow_mask)
    #cv2.imshow('yellow_mask', result)
    #cv2.imshow('yello', yellow_mask)
    # 黑色线条的阈值
    lower_black = np.array([100, 50, 0])
    ...
    yellow_pixels = cv2.countNonZero(yellow_mask[y_start:y_end, x_start:x_end])
    ...
    return yellow_area, black_area,yellow_pixels
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 正常巡线
        current_yellow_area, current_black_area, yellow_pixels = patrol_line(frame)
        ...
        if current_yellow_area > 1000 and current_black_area > 6000  and count_xiehuo == 0 and current_time - start_time >3 :
