...
def patrol_line(cv_image):
    ...
    #lower_black = np.array([120, 0, 0])
    #upper_black = np.array([255, 156, 95])
    ...
    # 处理heise线条的掩
    dilate_kernel1 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (1, 1))
...
def obstacle():
    for _ in range(2):
        ...
        time.sleep(0.5)
    for _ in range(2):
        turn_velocity = [0.0, 0.75]
        turn_yawspeed = 0.0
        if not debug:
            robot.robot_high_control(velocity=turn_velocity, yawSpeed=turn_yawspeed)
        time.sleep(0.5)
    for _ in range(3):
        turn_velocity = [0.87, 0.0]
        turn_yawspeed = -0.15
        if not debug:
            robot.robot_high_control(velocity=turn_velocity, yawSpeed=turn_yawspeed)
        time.sleep(0.5)
    for _ in range(2):
        turn_velocity = [0.2, -0.75]
        turn_yawspeed = 0.0
        if not debug:
            robot.robot_high_control(velocity=turn_velocity, yawSpeed=turn_yawspeed)
        time.sleep(0.5)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            # 台阶区     5000
            if current_black_area > 10000 and count_taijie == 0 and flag_xiehuo_over == 1 and turn_number == 2:
            ...
            if current_yellow_area < 10000 and flag_obstacle_over == 1 and count_over == 0:
                ...
                if current_time_over - obstacle_over_time > 1:
                    if not debug:
                        for _ in range(1):
                            robot.robot_high_control(velocity=[1.0, 0.0], yawSpeed=0.0)
                            time.sleep(0.5)
                        print("比赛结束...")
                    break
