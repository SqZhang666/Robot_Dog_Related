...
def find_connected_component_containing_point(image, point):

    # 查找连通域
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(image, connectivity=8)

    flag_img = cv2.circle(image, point, 0, (0, 255, 0), 10)
    cv2.imshow('find_connected_component_containing_point', flag_img)
    ...
    if found_label != -1:
        output_image[labels == found_label] = 255

    return output_image

def patrol_line(cv_image):
    height, width, channels = cv_image.shape
    cv2.imshow('chuli', cv_image)
    ...
    point = (240, 220)  # 指定的坐标
    yellow_mask = find_connected_component_containing_point(yellow_mask,point)
    ...
    # 在掩码图像上画出黄色线条的质心
    cv2.circle(yellow_mask, (int(cx_yellow), int(cy_yellow)), 0, (0, 255, 0), 10)
...
if __name__ == '__main__':
    # # # 视频文件路径
    # video_path = 'F:/2024/GO_Robcom/ceshi/robot6.22/下巴.avi'
    # # 打开视频文件
